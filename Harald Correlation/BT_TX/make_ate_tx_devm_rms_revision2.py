"""BT TX DEVM RMS -- ATE counterpart to make_bt_tx_devm_rms_revision2.py.
...
full rationale. Only the metric (ATE "DEVM_RMS", Bench Y_COL
evm_aver_aver-%) and the value-range/step lookup (DEVM's spec-anchored
...
derivation) -- added alongside FNAME_RE, not replacing it. For DEVM RMS
specifically: EDR-family packets (2DH5/3DH5/4DH5/8DH5) use test "DEVMRMS";
every other DEVM-bearing family (HDRPS2/HDRPM8/12/16/HDRPL16/24/32,
HDT-family via the HDT7P5 alias, UHDR32/48) uses "EVMRMS" instead --
...
import make_bt_tx_devm_rms_revision2 as devm_mod
...
ATE_PNG_DIR = PATHS.result_png_dir.parent / "BT_TX_ATE" / "devm_rms_revision2"
...
NEW_DEVM_RMS_TEST_NAMES = ("DEVMRMS", "EVMRMS")
    ...
    with path.open(encoding="utf-8-sig", newline="") as fh:
        ...
        for row in reader:
            ...
            if name.startswith("BTTX_"):
                ...
                if not m or m.group("domain") != "RF" or m.group("test") not in NEW_DEVM_RMS_TEST_NAMES:
                    continue
...
def get_ate_group_extents(bench, pa_slices_max, dig_gain_max):
    ...
    draw_all() and the Bench module's main() (see make_bt_tx_devm_rms_
...
def _draw_one(job):
    ...
    devm_mod.draw_png(values, title, "DEVM RMS (%)", png_path, value_range=value_range, step=step, spec=spec,
                       figsize=figsize)
    return name, item
