"""BT TX DEVM RMS revision2 distribution histograms.

Renamed from make_bt_tx_evm_revision2.py (2026-07-19) -- "EVM" and "DEVM"
mean the same thing in this project; all three DEVM-family metrics (RMS,
Peak, 99pct) now consistently use the "DEVM" name. Behavior is otherwise
unchanged: same pa_slices/dig_gain-fixed-at-max rule, same per-combo IQR
outlier handling (2026-07-18/19 fold-into-histogram rule), same spec-line
overlay convention.

For every (packet_type, pa_supply, pa_gain, frequency-MHz) combination,
collect every evm_aver_aver-% value from every DUT and draw a horizontal
bar-chart histogram:
  X axis = Count
  Y axis = DEVM RMS value bins
...
# header + Bench/ATE subheader + Mean+/-Std + Min/Max + USL (DEVM RMS's
# spec_constant=True merges LSL/USL into one row, and RMS has no LSL) --
...
def _font_scale(figsize) -> float:
    ...
    same reasoning (a fixed point size collides into unreadable overlap once
    figure_size_for() hands back a much shorter figure for a narrow-column
    PPT row)."""
    if figsize is None:
        return 1.0
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=PATHS.result_xlsx_dir / "devm_rms_revision2")
    p.add_argument("--png-dir", type=Path, default=PATHS.result_png_dir / "devm_rms_revision2")
...
def main() -> None:
    ...
    # EXACT same Y-axis span. Lazy/deferred import (not at module top-
    # level) to avoid a circular import -- make_ate_tx_devm_rms_revision2
    # imports this module as devm_mod; by the time main() runs, this
    # module is already fully initialized, so Python just returns it from
    # sys.modules. get_ate_group_extents() is called from BOTH this
    # main() and the ATE module's own draw_all(), so both compute the
    # identical widened range independently from the same two raw data
    # sources -- no file-based coordination needed.
    import make_ate_tx_devm_rms_revision2 as ate_devm_mod
    for group, (lo, hi) in ate_devm_mod.get_ate_group_extents(data, pa_slices_max, dig_gain_max).items():
        if group in packet_ranges:
            vmin, vmax = packet_ranges[group]
            packet_ranges[group] = (min(vmin, math.floor(lo)), max(vmax, math.ceil(hi)))
    ...
    # revision2.py's identical block for the full rationale (matches
    # build_ppt_pwr_evm_revision2.py's discover()/slide-layout grouping,
    # which works off the PNG filename alone, same for every metric).
