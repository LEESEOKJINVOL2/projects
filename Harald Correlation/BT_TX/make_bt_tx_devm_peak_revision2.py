"""BT TX DEVM Peak revision2 distribution histograms.

Sibling script to make_bt_tx_devm_rms_revision2.py -- identical pipeline,
different data column. Expanded 2026-07-19 from the earlier 3-packet-only
("EDR/HDR" = 3DH5/4DH5/8DH5) devm_peak metric inside
make_bt_tx_freqacc_revision2.py to the FULL 13-packet DEVM breadth (matching
DEVM RMS/99pct) -- the underlying evm_peak_aver-% column has data for all
13 packets, and pulling this out into its own standalone script keeps all
three DEVM-family metrics structurally consistent. Its spec limit is
confirmed 3DH5-only (see bt_tx_devm_spec_lookup.py); every other packet
simply won't show a spec line.

For every (packet_type, pa_supply, pa_gain, frequency-MHz) combination,
collect every evm_peak_aver-% value from every DUT and draw a horizontal
bar-chart histogram:
  X axis = Count
  Y axis = DEVM Peak value bins
...
# header + Bench/ATE subheader + Mean+/-Std + Min/Max + USL (DEVM Peak's
# spec is 3DH5-only, USL only -- see bt_tx_devm_spec_lookup.py) -- an
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--bt-tx-dir", type=Path,
                   default=PATHS.bench_dir)
    p.add_argument("--out-dir", type=Path,
                   default=PATHS.result_xlsx_dir / "devm_peak_revision2")
    p.add_argument("--png-dir", type=Path,
                   default=PATHS.result_png_dir / "devm_peak_revision2")
...
def main() -> None:
    ...
    # level) to avoid a circular import -- make_ate_tx_devm_peak_revision2
    ...
    import make_ate_tx_devm_peak_revision2 as ate_devm_mod
