"""BT TX ACP distribution charts.

For every unique combination of the 6 config columns
  cfg-carrier_01-packet_type, -frequency-MHz, -pa_supply,
  -pa_slices, -pa_gain, -dig_gain
plot acp_avg-dBm (Y) against meas-offset_frequency-MHz (X, -10..10),
...
Two outputs per combo:
  ...
  * PNG image in Result_PNG/.

File name encodes the combo, e.g.
  packet_type(1DH5)_frequency-MHz(2441)_pa_supply(0)_pa_slices(1)_pa_gain(0)_dig_gain(64)
(the "cfg-carrier_01-" prefix is stripped).
"""
...
X_COL = "meas-offset_frequency-MHz"
Y_COL = "acp_avg-dBm"
...
def read_all(bt_tx_dir: Path):
    """Return (duts, data) where data[key][dut][offset] = acp_avg."""
    ...
    for path in files:
        ...
        with path.open(encoding="utf-8-sig", newline="") as fh:
            ...
            for row in reader:
                off = to_float(row.get(X_COL, ""))
                if off is None:
                    continue
                acp = to_float(row.get(Y_COL, ""))
                if acp is None:
                    continue
                key = tuple((row.get(c, "") or "").strip() for c in KEY_COLS)
                data[key][dut][off] = acp
        print(f"  read {path.name} (DUT #{dut})")
    return sorted(duts), data


def offset_axis(data_for_combo: dict[int, dict[float, float]]) -> list[float]:
    offs: set[float] = set()
    for dut_map in data_for_combo.values():
        offs.update(dut_map.keys())
    return sorted(offs)
...
def write_png(png_path: Path, title: str, offsets, duts, combo_data) -> None:
    fig, ax = plt.subplots(figsize=(14, 7))
    for i, dut in enumerate(duts):
        ys = [combo_data.get(dut, {}).get(off) for off in offsets]
        ax.plot(offsets, ys, linewidth=1.0, color=f"#{color_hex(i, len(duts))}",
                label=f"#{dut}")
    ax.set_title(title, fontsize=9)
    ax.set_xlabel("Offset Frequency (MHz)")
    ax.set_ylabel("ACP (dBm)")
...
def write_xlsx(xlsx_path: Path, title: str, offsets, duts, combo_data) -> None:
    wb = Workbook()
    ws = wb.active
    ws.title = "ACP"
    ...
    # Data table: offset column + one column per DUT.
    ws.cell(data_header_row, 1, "Offset (MHz)")
    for j, dut in enumerate(duts, 2):
        ws.cell(data_header_row, j, f"#{dut}")
    for r, off in enumerate(offsets, data_header_row + 1):
        ws.cell(r, 1, off)
        for j, dut in enumerate(duts, 2):
            v = combo_data.get(dut, {}).get(off)
            if v is not None:
                ws.cell(r, j, v)
    ...
    last_row = data_header_row + len(offsets)
    chart = LineChart()
    chart.title = title
    chart.style = 10
    ...
    chart.height = 15
    chart.width = 34
    chart.y_axis.title = "ACP (dBm)"
    chart.x_axis.title = "Offset Frequency (MHz)"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=PATHS.result_xlsx_dir / "acp")
    p.add_argument("--png-dir", type=Path, default=PATHS.result_png_dir / "acp")
...
def main() -> None:
    ...
    for n, key in enumerate(keys, 1):
        ...
        title = f"BT_TX_{name}"
        offsets = offset_axis(data[key])
        write_xlsx(args.out_dir / f"{fname}.xlsx", title, offsets, duts, data[key])
        write_png(args.png_dir / f"{fname}.png", title, offsets, duts, data[key])
