...
def read_all(bt_tx_dir: Path):
    """Return (duts, data) where data[key][dut][freq] = power_aver-dBm."""
    ...
    for path in files:
        ...
        with path.open(encoding="utf-8-sig", newline="") as fh:
            ...
            for row in reader:
                ...
                if freq is None:
                    continue
                power = to_float(row.get(Y_COL, ""))
                if power is None:
                    continue
                key = tuple((row.get(c, "") or "").strip() for c in KEY_COLS)
                data[key][dut][freq] = power
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir", type=Path, default=PATHS.result_xlsx_dir / "pwr")
    p.add_argument("--png-dir", type=Path, default=PATHS.result_png_dir / "pwr")
