"""Give BT_RX the per-config summary feature just added to BT_TX
(BT_TX/update_bt_tx_summary_by_vendor.py, the primary template this mirrors
closely), per user request 2026-08-25 ("TX에서 한거 동일하게 RX에도 적용해줘"
-- apply what was just done for BT_TX identically to BT_RX):

1. Add one "<Config> Summary" sheet per DUT config group -- same Mean/Std/
   Min/Max/USL/LSL (+ Delta) shape as the existing pooled "BT_RX Summary"
   ...
   the already-built "BT_RX Summary" sheet's own cells rather than
...
3. Sheet order: "BT_RX Summary", then one "<Config> Summary" sheet per
   sorted config group, then "Raw Values" last -- NOT hardcoded to any fixed
   group count (same past-bug-avoidance rule BT_TX's own module docstring
   documents: this derives the group set purely from whatever Config labels
   40pcs_config.txt actually contains, sorted for determinism).

Reuses the SAME 40pcs_config.txt file BT_TX already reads (same directory
convention -- see CONFIG_PATH below) -- confirmed this dataset's 40 real
DUT serials (e.g. "725HW3001RE0001A6Q") are the identical physical 40 DUTs/
serials BT_TX's own config file already maps (SNKJ:25 / DNNJ:15) -- no
BT_RX-specific config file or edits needed.

Why config, not vendor-folder: same reasoning as BT_TX -- BT_RX's Bench
folder is nested by DUT (Bench/#NN_<serial>/...), one folder per DUT, with
no folder-based way to recover group membership; group membership instead
comes from 40pcs_config.txt's Num/2DID/Config columns.

Reuses add_bt_rx_raw_values_sheet.py's collect_raw_rows/flatten_rows/
write_flat_raw_values_sheet for the actual per-DUT Bench/ATE data and the
flat-sheet fallback -- never re-derives a stat or duplicates that traversal.
"""
...
import add_bt_rx_raw_values_sheet as raw_sheet

PATHS = bt_rx_paths.get_paths("BT_RX")
REPORT_DATE = datetime.date.today().isoformat()
SUMMARY_PATH = PATHS.report_dir / f"{REPORT_DATE}_BT_RX_Summary.xlsx"

# Same one-level-up convention every other BT_RX/BT_TX script uses to locate
# a file shared with the rest of the workspace (paths.py, config.xml, the
# 40pcs_config.txt itself) -- resolves to the root copy when run from root
# BT_RX/, and to UI's copy when run from UI/BT_RX/, since _BASE_DIR is this
# script's own parent directory's parent (root, or UI/) in either location.
...
def spec_by_name_from_summary_sheet(wb) -> dict:
    ...
    straight off the already-built "BT_RX Summary" sheet's own cells --
...
def build_config_summary_rows(dict_rows, config_serials: set, spec_by_name: dict) -> list:
    """Same shape as build_bt_rx_xlsx_summary's own rows (name, Bench Mean/
...
def write_config_summary_sheet(wb, sheet_name: str, rows: list, n_cols: int = 17) -> None:
    ...
    writer.write_summary_xlsx, mirrors update_bt_tx_summary_by_vendor.py's
    write_config_summary_sheet exactly. Does NOT save `wb`."""
...
def write_grouped_raw_values_sheet(wb, dict_rows, config_order, config_to_serials) -> None:
    ...
    update_bt_tx_summary_by_vendor.py's own Raw Values rebuild exactly."""
    rv_rows = []
    for name, bench_by_serial, ate_by_serial in dict_rows:
        row = [name]
...
def fallback_flat_raw_values(wb, num_to_serial) -> None:
    ...
    sheets are written in this path either. Does NOT save `wb`."""
    ate_serials = raw_sheet.collect_ate_serials()
    all_serials = sorted(set(num_to_serial.values()) | ate_serials)
    dict_rows = raw_sheet.collect_raw_rows(num_to_serial)
...
def main() -> None:
    ...
    if not num_to_serial:
        print("Bench folder has no embedded DUT serials (old flat layout) -- "
              ...
              "entirely (same as add_bt_rx_raw_values_sheet.py's own rule).")
        return
    ...
    print("Reading USL/LSL off the existing 'BT_RX Summary' sheet (spec "
    ...
    desired_order = ["BT_RX Summary", *(f"{c} Summary" for c in config_order), "Raw Values"]
