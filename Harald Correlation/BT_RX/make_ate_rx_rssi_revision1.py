"""BT_RX RSSI -- ATE counterpart to make_bt_rx_rssi_revision1.py. Must run
AFTER that script (imports its build_jobs()/title_for()/audit helpers
directly, same convention as make_ate_tx_pwr_revision2.py importing
make_bt_tx_pwr_revision2.py in BT_TX).

Draws the ATE-side PNG for every combo make_bt_rx_rssi_revision1.py already
matched to a Bench DUT (the "excluded entirely" combos -- no Bench data
within tolerance -- are already dropped by build_jobs() itself, so nothing
...
import make_bt_rx_rssi_revision1 as bench_mod
