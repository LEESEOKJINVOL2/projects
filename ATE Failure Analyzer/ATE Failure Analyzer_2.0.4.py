        ...
        try:
            ...
            for index, row in data.iterrows():
                if row['SerialNumber'] in ['Site', 'TestResult', 'hbin#', 'sbin#', 'Durant ECID', 'startTime', 'stopTime', 
                    'testTime(ms)', 'testTime(s)', 'DeviceName', 'Device_Name', 'HandlerID', 'TesterID', 'TestPGM', 'LotID', 'ProcessID', 'OperatorID', 
                    'Loadboard#', 'RetestCode', 'ToteID', 'Temperature', 'Barcode_Err', 'Barcode Err', # common
                    
                    'LBID_X_X_X_X_X_X_X_NV_X', 'HOSTID_X_X_X_X_X_X_X_NV_X', 'WSPS_ENABLE_X_X_X_X_X_X_NV_X', 
                    'LOAD_PMCAL_TABLE_X_X_X_X_X_NV_X', 'LOAD_KGDCAL_TABLE_X_X_X_X_X_NV_X', 
                    'DURANT_LOTID_X_X_X_X_X_X_NV_X', 'LAGUNA_LOTID_X_X_X_X_X_X_NV_X', # Elder & Younger

                    'DURANT_LOTID_X_X_X_X_X_X', 'FWSTSLEP_CHIP_JTAG_HASH_L_X_X_X', # Tiny
                    
                    'CHIP_ID_Previous',
                    'CHIP_ID_Current',
                    'CHIP_ID', 
                    'GPIO0~GPIO7', 'GPIO8~GPIO15', 
                    'OTP_Verification', 
                    'VBAT_CCADC_iBAT_read_0x7c10', 'VBAT_CCADC_iBAT_read_0x7c10.1', 
                    'VBAT_CCADC_iBAT_read_0x7c12', 'VBAT_CCADC_iBAT_read_0x7c12.1', 'VBAT_CCADC_iBAT_read_0x7c12.2', 
                    'VBAT_GPADC_read_1', 'VBAT_GPADC_read_1.1', 'VBAT_GPADC_read_2', 'VBAT_GPADC_read_2.1' # Watt 

                    ]: # trash data
                    
                    continue # skip
                elif row['SerialNumber'] in ['hbin', 'sbin']: # add valid string data
                    ws.append(list(row))
                else:
...
def main():
    root = tk.Tk()
    root.title("ATE Failure Analyzer_2.0.4")
...
# 20240604 ATE Failure Analyzer 2.0.1
# add Watt OS test
# delete 'Barcode_Err' index

# 250602 ATE Failure Analyzer 2.0.4
# update Watt pgm